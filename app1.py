...
# --- CUADRO PARA MEDIR POSICIÓN POR TIEMPO ---
st.sidebar.markdown("---")
st.sidebar.header("📏 Medir Posición por Tiempo")
st.sidebar.write("Ingresa un tiempo exacto de vuelo para saber dónde se encuentra el balón.")

# El usuario ingresa un tiempo en segundos (el máximo es el tiempo total de vuelo)
# El usuario ingresa un tiempo en segundos (SIN LÍMITE MÁXIMO ESTRICTO)
t_target = st.sidebar.number_input(
    "Tiempo de vuelo (s)", 
    min_value=0.0, 
    max_value=1000.0, # <-- Aquí aumentamos el límite a 1000 segundos
    value=float(t_vuelo / 2) if t_vuelo > 0 else 0.0, 
    step=0.1
)

# Calculamos la distancia (X) y la altura (Y) para ese segundo exacto
x_target = v0 * np.cos(angle_rad) * t_target
y_target = v0 * np.sin(angle_rad) * t_target - (0.5 * g * t_target**2)

# Opcional: Evitar que muestre alturas negativas si el balón ya tocó el suelo (solo en gravedad normal)
if g > 0 and y_target < 0:
    y_target = 0.0

# Mostramos el cuadro verde con los resultados
st.sidebar.success(f"""
**Datos en el segundo t = {t_target:.2f} s:**
* 📏 **Distancia en X:** {x_target:.2f} m
